...
- Wrapper stdin contract: bare keystrokes are forwarded to the child but do **not** flip the run_state. The card stays surfaced until either the AI itself emits real semantic output (`report` / `stdout` / `stderr` traffic from a Stop hook or headless adapter) or the user sends an instruction through the reply box (`steer send`). Esc (`0x1B`) or Ctrl-C (`0x03`) still flips back to `state=waiting` so a user who started typing and changed their mind explicitly resurfaces the card. The agent does not maintain any synthetic `[user] (typed in terminal)` transcript entry; visibility is decided purely from `run_state` + traffic existence.
