...
> **One-liner**: 여러 AI CLI 세션의 보고를 한 대화방처럼 받고, 필요한 순간 바로 지시해서, AI 작업이 멈추지 않게 하는 개인용 operations room.
...
**가설**: 여러 AI 작업 세션을 메시징 앱처럼 한 곳에서 보고 받고 지시할 수 있으면, 사용자의 운영 latency가 줄고 AI idle time이 감소한다. 이 효과는 answer latency, instruction latency, idle time reduction, active session continuation rate로 측정 가능하다.
...
기존 솔루션 = "데스크톱 챗/터미널 환경 -> 폰으로 옮김" (mirroring). Steer = "여러 AI 작업 세션 -> 메시징 앱처럼 운영" (AI operations room).

제품의 핵심은 "결정만 빨리 답하기"가 아니라, 사용자가 여러 CLI 세션의 보고를 받고, 막힌 세션에 지시하고, 완료된 세션에 다음 작업을 이어붙여 AI가 멈추지 않게 하는 것이다.

비유: Instagram DM의 가벼움 + Telegram의 여러 방/구성 유연성 + Linear의 작업 상태/우선순위. 사용자는 AI 에이전트들과 DM하듯 대화하지만, 내부적으로는 각 CLI session의 상태와 command channel을 관리한다.

데이터 모델은 사용자-에이전트 세션의 1:1 대화 N개다. UX 모델은 기본적으로 unified room/control room이지만, 반드시 하나의 대화방이어야 하는 것은 아니다. 사용자는 원하면 여러 room으로 나눠 관리할 수 있다. 어떤 CLI session을 어떤 room에 초대할지는 후속 스펙으로 둔다.

## Differentiation

- **vs Anthropic Remote Control**: RC는 Claude 세션을 폰에 mirror하는 모델. Steer는 여러 CLI 세션을 room 기반으로 운영하고, 보고/지시/상태 관리를 한 흐름으로 묶는다.
- **vs Happy**: Happy는 풀 챗을 폰으로 옮기는 검증된 wrapper/remote-control 인프라. Steer는 그 위에 "AI operations room" UX를 얹는다.
...
핵심 차별화: AI operations room, room 기반 multi-session 관리, wrapper-owned pty를 통한 실제 답/지시 주입, 빠른 답/빠른 지시 버튼, idle/block/running 상태와 throughput 메트릭.

## Core Experience

**Onboarding**: Mac 앱 설치 -> wrapper command 설치 (`steer claude`, `steer codex`) -> 로컬 권한/notification 설정 -> 기본 room 생성 -> 첫 CLI session 연결.

**Main loop**:

1. 사용자가 데스크톱에서 `steer claude` 등으로 AI CLI 세션을 시작한다.
2. Steer가 세션 출력과 상태를 캡처한다.
3. 세션이 진행 보고, 완료, blocker, decision, idle 상태에 도달하면 room에 메시지로 올라온다.
4. 분류 AI가 메시지를 요약하고, 필요한 경우 quick reply/quick instruction 옵션을 만든다.
5. 사용자는 폰/Mac에서 버튼 탭 또는 짧은 텍스트로 응답한다.
6. 답변 또는 지시가 해당 CLI session에 주입되고, 세션은 즉시 작업을 계속한다.
7. 사용자는 기존 질문에 답하는 것뿐 아니라, composer에서 먼저 특정 session에 새 지시를 보낼 수 있다.

## Design Direction

- **Primary**: Instagram DM. 가볍고 즉각적인 대화방, compact bubbles, unread/read, quick reply, mobile-first interaction.
- **Secondary**: Telegram. 여러 room, room별 session 구성, flexible chat structure.
- **Technical layer**: Linear. `running`, `waiting`, `blocked`, `done` 같은 세션 상태, priority, project grouping, dense-but-clean metadata.
- **Optional Mac interaction**: Raycast. command palette, `@session` routing, 빠른 room/session 전환.
...
- **Week 1**: Mac 단독 prototype. Wrapper로 stdout/stderr 캡처 + stdin 주입 검증. 기본 room UI, session status, report/instruct loop.
- **Week 2**: Mac UX 다듬기. room list/session filter, composer target routing, notification, idle/block 상태 메트릭.
- **Week 3-4**: iOS 앱 + 동기화. iOS DM-like room UI + 푸시 알림. 5명 베타.
